...
class JetMax:
    ...
    def set_servo(self, servo_id, pulse, duration):
        ...
        duration = int(duration * 1000)
        hiwonder.serial_servo.set_position(servo_id, int(pulse), duration)
        servos = list(self.servos)
        servos[servo_id - 1] = pulse
        self.servos = tuple(servos)
        self.angles = hiwonder.kinematic.pulse_to_deg(self.servos)
        self.position = hiwonder.kinematic.forward_kinematics(self.angles)
        return True
